            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            <?php $no = 1; ?>
                            @foreach($pesanans_details as $pesanans_detail)
                            <tr>
                                <td>{{ $no++ }}</td>
                                <td>
                                    <img src="{{ url('uploads') }}/{{ $pesanans_detail->katalogs->gambar }}" width="100" alt="...">
                                </td>
                                <td>{{ $pesanans_detail->katalogs->nama_katalogs }}</td>
                                <td>{{ $pesanans_detail->jumlah }} kain</td>
                                <td>
                                    <form action="{{ url('check-out') }}/{{ $pesanans_detail->id }}" method="post">
                                        @csrf
                                        {{ method_field('DELETE') }}
